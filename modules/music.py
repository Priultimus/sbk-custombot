...
class Music:
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):
            lavalink.Client(bot=bot, password='urmomgaylol',
                            loop=self.bot.loop, log_level=logging.DEBUG)
    ...
    async def track_hook(self, event):
        if isinstance(event, lavalink.Events.TrackStartEvent):
            ...
            if c:
                ...
                if c:
                    embed = discord.Embed(colour=c.guild.me.top_role.colour,
                                          title='Now Playing',
    ...
    async def play(self, ctx, *, query):
        ...
        if not player.is_connected:
            ...
            if not ctx.author.voice or not ctx.author.voice.channel or \
             player.connected_channel.id != ctx.author.voice.channel.id:

...
class General:
    ...
    async def on_member_join(self, member):
        if test:
            ...
            mainsbk = discord.utils.get(self.bot.guilds, id=448743173213126666)
            channel = discord.utils.get(mainsbk.channels, id=448743173217320961)
        message = DataManager.read('data/general.json')['welcome']
        await channel.send(message + f"{member.mention}!")
        role = discord.utils.get(ctx.guild.roles, name='Member')
        await member.add_roles(role, reason=f"Joined; added Member")
